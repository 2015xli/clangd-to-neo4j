...
class _ClangWorkerImpl:
    ...
    def run(self) -> Tuple[List[Dict], Set[Tuple[str, str]]]:
    ...
    def _parse_translation_unit(self, file_path: str):
        ...
        if self.entry['clang_include_path']: sanitized_args.append(f"-I{self.entry['clang_include_path']}")
    ...
    def _walk_ast(self, node):
        ...
        if not file_name or not file_name.startswith(self.entry['project_path']):
            return
...
class _TreesitterWorkerImpl:
    ...
    def run(self) -> Tuple[List[Dict], Set]:
        try:
            ...
            return [{"FileURI": f"file://{os.path.abspath(self.file_path)}", "Functions": functions}], set()
        except Exception as e:
            logger.error(f"Treesitter worker failed to parse {self.file_path}: {e}")
            return [], set()


def _parallel_worker(parser_type: str, data: Any) -> Tuple[List[Dict], Set]:
    """Generic top-level worker function that dispatches to the correct implementation."""
    # Increase recursion limit for this worker process to handle deep ASTs
    sys.setrecursionlimit(3000)

    try:
        if parser_type == 'clang':
            worker_impl = _ClangWorkerImpl(data)
        elif parser_type == 'treesitter':
            worker_impl = _TreesitterWorkerImpl(data)
        else:
            raise ValueError(f"Unknown parser type: {parser_type}")
        return worker_impl.run()
...
class CompilationParser:
    ...
    def _parallel_parse(self, items_to_process: List, parser_type: str, num_workers: int, desc: str):
        """Generic parallel processing framework."""
        all_spans = []
        all_includes = set()
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            # Pass parser_type along with the data item
            future_to_item = {executor.submit(_parallel_worker, parser_type, item): item for item in items_to_process}
...
class ClangParser(CompilationParser):
    ...
    def __init__(self, project_path: str, compile_commands_path: str):
        ...
        try: self.db = clang.cindex.CompilationDatabase.fromDirectory(db_dir)
        except clang.cindex.CompilationDatabaseError as e: logger.critical(f"Error loading compilation database from '{db_dir}': {e}"); raise

        self.clang_include_path = self._get_clang_resource_dir()

    def _get_db_dir(self, path_str: str) -> str:
        path = Path(path_str).resolve()
        if path.is_dir(): return str(path)
        if path.is_file() and path.name == "compile_commands.json": return str(path.parent)
        raise FileNotFoundError(f"compile_commands.json not found at or in {path_str}")
    ...
    def parse(self, files_to_parse: List[str], num_workers: int = 1):
        ...
        if num_workers and num_workers > 1:
            ...
            self._parallel_parse(compile_entries, 'clang', num_workers, "Parsing TUs (clang)")
        else:
            ...
            for entry in tqdm(compile_entries, desc="Parsing TUs (clang)"):
                spans, includes = _parallel_worker('clang', entry)
...
class TreesitterParser(CompilationParser):
    ...
    def parse(self, files_to_parse: List[str], num_workers: int = 1):
        ...
        if num_workers and num_workers > 1:
            ...
            self._parallel_parse(valid_files, 'treesitter', num_workers, "Parsing spans (treesitter)")
        else:
            ...
            for file_path in tqdm(valid_files, desc="Parsing spans (treesitter)"):
                spans, _ = _parallel_worker('treesitter', file_path)
                if spans: self.function_spans.extend(spans)
